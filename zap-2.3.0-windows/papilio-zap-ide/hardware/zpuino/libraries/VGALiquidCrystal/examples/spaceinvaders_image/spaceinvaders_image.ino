  ...
  Demonstrates moving from a 16x2 LCD display to VGA with splash screen and background image.
...
void hd44780_draw()
{
  const int display_x0 = 35;
  const int display_width = 100;
  const int display_y0 = 20;
  const int display_height = 20;  
  
  //Draw the PCB Green
//  VGA.setColor(GREEN);
//  VGA.drawRect(display_x0-10,display_y0-10,display_width+15,display_height+15);  
//  
//  //Draw the black background behind board
//  VGA.setColor(BLACK);
//  VGA.drawRect(display_x0-4,display_y0-4,display_width+5,display_height+5);
  
  //Draw the light green background behind display
  VGA.setColor(BLACK);
  VGA.drawRect(display_x0-2,display_y0-2,display_width,display_height);  
